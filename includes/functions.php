...
{
    return db()->query('SELECT id, name, bus_number, total_seats FROM buses ORDER BY id ASC')->fetchAll();
}

function get_bus_by_id(int $busId): ?array
{
    $stmt = db()->prepare('SELECT id, name, bus_number, total_seats FROM buses WHERE id = :id LIMIT 1');
